...
| SVC-20260728-01 | Object system facade re-export | Root facade | active | Root `engine::object` re-exports supported renderer object types; `engine::camera` re-exports `EditorCamera`; `engine::command` re-exports concrete object commands. Root `engine::prelude` includes only common `ObjectId`, `ObjectKind`, `SceneObjectId`, `Selection`, `SelectionChange`, `ObjectQueryFilter`, and `RayHit` imports. Dependency direction is root->renderer only; renderer never depends on root. `engine_events` owns vocabulary types (`ObjectKind`, `SceneObjectId`). | Same dependency rule as other re-export modules. | `cargo check`, `cargo test -p engine --test facade_imports`. | `DECISION-20260728-01` |
| SVC-20260728-02 | Game primitives app-owned bridge edge | Apps/examples | active | App-owned bridges (physics, audio, scripting) consume renderer neutral DTOs but never create a renderer→physics, renderer→audio, or renderer→scripting dependency edge. The bridge pattern is app-local; the renderer exports only Vulkan-free DTOs. No new workspace crate dependencies are added in this phase. | Same dependency rule as existing dogfood bridge and BSP coordinator. | `cargo check`, `cargo tree` edge audit. | `DECISION-20260728-06` |
