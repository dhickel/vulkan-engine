...
| SVC-20260728-01 | Object system facade re-export | Root facade | active | Root `engine::object` re-exports `renderer::object` public types. Root `engine::prelude` includes `ObjectId`, `ObjectKind`, `SceneObjectId`, `Selection`, `SelectionChange`. Dependency direction is root->renderer only; renderer never depends on root. `engine_events` owns vocabulary types (`ObjectKind`, `SceneObjectId`). | Same dependency rule as other re-export modules. | `cargo check`, `cargo test -p engine --test facade_imports`. | `DECISION-20260728-01` |
