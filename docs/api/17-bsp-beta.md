...
| [#58](https://github.com/dhickel/vulkan-engine/issues/58) | Generated faces lack baked data; runtime falls back unlit, compiler/release evidence remains blocked |
...
All crate-level tests pass (`bsp`, `bsp_runtime`, `bsp_generator`, `bsp_beta` unit tests). A structurally valid unresolved texture slot uses the diagnostic texture and a missing lightmap uses the unlit fallback in either import mode; release/compiler evidence still requires complete authored resources. Visual acceptance, reference calibration, and live WSI evidence require a GPU/WSI environment and have not yet been produced.
