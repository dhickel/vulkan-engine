...
- `render_scene`, `render_scene_headless`, `begin_frame`, and `end_frame` emit `FrameStarted`/`FrameEnded`.
...
- `RuntimeEvents` owns a bounded event bus.
...
- `apps/dungeon_dogfood/src/events.rs` installs the same style of app-side logger.
