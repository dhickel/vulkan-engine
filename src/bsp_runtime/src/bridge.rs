...
impl BridgeAggregator {
    ...
    pub fn prepare(
        ...
        for (i, entry) in self.bridges.iter_mut().enumerate() {
            ...
            match result {
                Ok(Ok(token)) => {
                    self.prepared_tokens[i] = Some(token);
                }
                Ok(Err(msg)) => {
        ...
        if !failures.is_empty() {
            let rollback_failures = self.rollback_prepared();
    ...
    /// Roll back only those bridges that have prepared tokens.
    fn rollback_prepared(&mut self) -> Vec<BridgeFailure> {
        let mut failures = Vec::new();
        for (i, entry) in self.bridges.iter_mut().enumerate() {
            if let Some(token) = self.prepared_tokens[i].take() {
                let result = catch_unwind(AssertUnwindSafe(|| {
                ...
                if result.is_err() {
                    log::error!("bridge '{}' panicked during rollback", entry.name);
                    failures.push(BridgeFailure::new(
                        entry.name.clone(),
                        BridgePhase::Rollback,
                        "bridge panicked during rollback".to_string(),
