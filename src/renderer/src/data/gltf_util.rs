...
use crate::data::gpu_data::{
    EmissiveMap, MeshMeta, MeshPrimitive, Node, NodeMeta, NormalMap, OcclusionMap, SurfaceMeta, TextureMeta,
    Transform, Vertex, VkGpuTextureBuffer,
};
use ash::vk;
use glam::Vec2;
...
pub fn parse_gltf_to_raw(
    ...
    let mut global_indices = Vec::<u32>::new();
    let mut global_vertices = Vec::<Vertex>::new();
    let mut mesh_primitives_map = HashMap::<usize, Vec<MeshPrimitive>>::with_capacity(gltf_data.meshes().count());
    ...
    for mesh in gltf_data.meshes() {
        ...
        for primitive in mesh.primitives() {
            ...
            let vertex_start = global_vertices.len();
            let index_start = global_indices.len();

            // INDICES
            let initial_idx_count = global_indices.len();
            if let Some(indices_reader) = reader.read_indices() {
                parse_indices(indices_reader, vertex_start, &mut global_indices);
            } else {
                return Err("No indices found".to_string());
            }
            let index_count = global_indices.len() - initial_idx_count;

            // VERTICES
            if let Some(vertex_reader) = reader.read_positions() {
                parse_vertices(vertex_reader, &mut global_vertices);
            } else {
            ...
            if let Some(normal_reader) = reader.read_normals() {
                parse_normals(normal_reader, vertex_start, &mut global_vertices);
            } else {
                info!("Generating smooth normals for surface in mesh: {:?}", name);
                generate_smooth_normals(
                    &mut global_vertices, // Pass whole array as indices are array indices
                    &global_indices[index_start..index_start + index_count],// Use only applicable surface indices
                );
            }

            // UV COORDS
            if let Some(uv_reader) = reader.read_tex_coords(0) {
                parse_texture_coords(uv_reader, vertex_start, &mut global_vertices);
            } else {
            ...
            if let Some(tangent_reader) = reader.read_tangents() {
                parse_tangents(tangent_reader, vertex_start, &mut global_vertices);
            } else {
                info!("Generating mikktspace tangents for surface in mesh: {:?}", name);
                let mut primitive = Primitive {
                    vertices: &mut global_vertices,
                    indices: &global_indices[index_start..index_start + index_count],
                };
            ...
            if let Some(color_reader) = reader.read_colors(0) {
                parse_colors(color_reader, vertex_start, &mut global_vertices);
            } else {
                info!("Using default color of (1.0, 1.0, 1.0, 1.0) for surface in mesh: {:?}", name);
            }

            let mat_idx = primitive.material().index().map(|idx| idx as u32);
            let material_id = mat_idx.and_then(|i| mapped_materials.get(&i)).copied().unwrap_or(TextureCache::DEFAULT_MAT_ROUGH_MAT);

            primitives.push(MeshPrimitive {
                mesh_id: 0, // Placeholder
                first_index: index_start as u32,
                index_count: index_count as u32,
                material_id,
            });
        }
        mesh_primitives_map.insert(mesh.index(), primitives);
    }

    let mega_mesh = MeshMeta {
        name: format!("GLTF_{}", path),
        indices: global_indices,
        vertices: global_vertices,
        material_index: None,
    };

    let mega_mesh_id = mesh_cache.add(mega_mesh);

    // Update mesh_id
    for primitives in mesh_primitives_map.values_mut() {
        for p in primitives {
            p.mesh_id = mega_mesh_id;
        }
    }
    ...
    let (parsed_nodes, top_node_indices) = parse_nodes(&gltf_data, &mesh_primitives_map);
...
fn map_materials(
    ...
    for material in parsed_materials {
        ...
        let mut mat_data = gpu_data::MaterialMeta::default();
        ...
        mat_data.material_values.alpha_mask = alpha_mode.to_float_value();
        mat_data.material_values.alpha_mask_cutoff = material.alpha_cutoff().unwrap_or(0.5);

        if let Some(bc_id) = base_color_tex_id {
            ...
            // FIXME: GLTF texture has tex_coord() which returns u32 set index.
            // But we need to check if it's 0 or 1.
            let uv_set = 0; // Assuming set 0 for now
            mat_data.add_base_color(texture_id, bc_factor, uv_set);
        } else {
            log::info!("Used Default Base Color For Material: {:?}", name);
            mat_data.add_base_color(TextureCache::DEFAULT_COLOR_TEX, glam::vec4(1.0, 1.0, 1.0, 1.0), 0);
        };
        ...
        if let Some(mr_id) = metallic_roughness_tex_id {
            ...
            let texture_data = gpu_data::TextureMeta::from_gltf_texture(data);
            let texture_id = texture_cache.add_texture(texture_data);
            mat_data.add_metallic_roughness(texture_id, met_factor, rough_factor, 0);
        } else {
            log::info!("Used Default Metallic Roughness For Material: {:?}", name);
             mat_data.add_metallic_roughness(TextureCache::DEFAULT_ROUGH_TEX, 0.0, 1.0, 0);
        };

        if let Some(norm_id) = normal_id {
            ...
            let texture_data = gpu_data::TextureMeta::from_gltf_texture(data);
            let texture_id = texture_cache.add_texture(texture_data);
            mat_data.add_normal(texture_id, scale, 0);
        }

        if let Some(occ_id) = occlusion_id {
            ...
            let texture_data = gpu_data::TextureMeta::from_gltf_texture(data);
            let texture_id = texture_cache.add_texture(texture_data);
            mat_data.add_occlusion(texture_id, strength, 0);
        }

        if let Some(e_id) = emissive_id {
            ...
            let texture_data = gpu_data::TextureMeta::from_gltf_texture(data);
            let texture_id = texture_cache.add_texture(texture_data);
            mat_data.add_emissive(texture_id, factor, 1.0, 0); // Strength 1.0?
        }
...
fn parse_nodes(
    gltf_data: &Document,
    mesh_primitives_map: &HashMap<usize, Vec<MeshPrimitive>>,
    ...
    for node in gltf_data.nodes() {
        ...
        let primitives = if let Some(mesh) = node.mesh() {
            mesh_primitives_map.get(&mesh.index()).cloned().unwrap_or_default()
        } else {
            Vec::new()
        };
    ...
    let root_children = top_node_indices
        ...
        .map(|&node_index| {
            ...
            let root_child = Rc::new(RefCell::new(gpu_data::Node {
                ...
                children: vec![],
                primitives: parent_node.primitives.clone(),
                world_transform: parent_node.og_matrix,
                local_transform: parent_node.local_transform.compose(),
                dirty: true,
            }));
    ...
    children
        ...
        .map(|&child_index| {
            ...
            let child_node = Rc::new(RefCell::new(gpu_data::Node {
                ...
                children: vec![],
                primitives: child_meta.primitives.clone(),
                world_transform: child_meta.og_matrix,
                local_transform: child_meta.local_transform.compose(),
                dirty: true,
            }));
